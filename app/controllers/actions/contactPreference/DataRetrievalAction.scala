...
class DataRetrievalActionImpl @Inject()(
  ...
  override protected def transform[A](request: IdentifierRequest[A]): Future[OptionalDataRequest[A]] = {
    ...
    contactPreferencesService.get(request.enrolmentVpdId)(headerCarrier).map {
      case Left(_)    => OptionalDataRequest(request, request.enrolmentVpdId, request.internalId, request.credId, None)
      case Right(ua)  => OptionalDataRequest(request, request.enrolmentVpdId, request.internalId, request.credId, Some(ua))
    }
  }
}
