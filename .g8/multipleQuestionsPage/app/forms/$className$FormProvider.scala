...
class $className$FormProvider @Inject() extends Mappings {
   ...
   def apply(): Form[$className$] = Form(
     mapping(
      "$field1Name$" -> textStrippingSpaces("$className;format="decap"$.error.$field1Name$.required")
        .verifying(maxLength($field1MaxLength$, "$className;format="decap"$.error.$field1Name$.length")),
      "$field2Name$" -> textStrippingSpaces("$className;format="decap"$.error.$field2Name$.required")
