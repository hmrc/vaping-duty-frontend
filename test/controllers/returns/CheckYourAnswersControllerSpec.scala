...
class CheckYourAnswersControllerSpec extends SpecBase {
  ...
  "ReturnsCheckYourAnswers Controller" - {
    ...
    "must submit and return the correct response for a POST" in {
      ...
      running(application) {
        val request = FakeRequest(POST, controllers.returns.routes.CheckYourAnswersController.onSubmit().url)

        val result = route(application, request).value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustBe controllers.returns.routes.ConfirmationController.onPageLoad(Some("vpdReferenceNumber")).url
      }
    }
