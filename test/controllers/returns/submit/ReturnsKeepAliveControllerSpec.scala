...
class ReturnsKeepAliveControllerSpec extends SpecBase with MockitoSugar {
    ...
    "when the user has answered some questions" - {
      ...
      "must keep the answers alive and return OK" in {
        ...
        running(application) {
          ...
          val request = FakeRequest(GET, controllers.returns.submit.routes.ReturnsKeepAliveController.keepAlive().url)
