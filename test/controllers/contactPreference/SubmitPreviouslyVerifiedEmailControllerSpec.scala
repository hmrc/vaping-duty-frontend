...
class SubmitPreviouslyVerifiedEmailControllerSpec extends SpecBase {

  "SubmitPreviouslyVerifiedEmail Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.SubmitPreviouslyVerifiedEmailController.onPageLoad().url)
    ...
    "must redirect to journey recovery when email verification service fails" in {
      ...
      running(application) {
        val request = FakeRequest(POST, routes.SubmitPreviouslyVerifiedEmailController.onSubmit().url)
