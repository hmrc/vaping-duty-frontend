...
class ReturnsUserAnswersSpec extends SpecBase {
  val ua = ReturnsUserAnswers(vpdId = vpdId.toString, periodKey = optPeriodKey, startedTime = Instant.now(clock), lastUpdated = Instant.now(clock))
  ...
  "ReturnsUserAnswers" - {
    ...
    val errorJson =
      s"""{"vpdId":"$vpdId","lastUpdated":{"$$date":{"$$numberLong":"1718118467838"}}}"""
