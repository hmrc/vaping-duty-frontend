...
class ReturnsUserAnswersSpec extends SpecBase {
  val ua = ReturnsUserAnswers(vpdId = vpdId.toString, periodKey = periodKey.toString, startedTime = Instant.now(clock), lastUpdated = Instant.now(clock))
  ...
  "ReturnsUserAnswers" - {
    val json =
      s"""{"vpdId":"$vpdId","periodKey":"26AF","data":{},"startedTime":{"$$date":{"$$numberLong":"1770894786000"}},"lastUpdated":{"$$date":{"$$numberLong":"1770894786000"}}}"""
    ...
    "must show errors if json is not in the correct structure" in {
      val result = Json.parse(errorJson).validate[ReturnsUserAnswers]
    ...
    "must deserialise from json" in {
      Json.parse(json).as[ReturnsUserAnswers] mustBe ua
    }
  }
}